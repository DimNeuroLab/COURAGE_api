...
async function create_tweets(topic='experiment') {
    ...
    for (let tweet of loaded_tweets['tweets']) {
         ...
         var fake_name = tweet['id_str'] + '_fake'
         ...
         s = '<div id="analysis-' + tweet['id_str'] + '" class="analysis">' +
             ...
             '<canvas class="analysis-canvas" id=' + fake_name + '></canvas>' +
             '</div>' +
             '<div class="post-analysis-canvas">' +
    ...
    for (let tweet_idx = 0; tweet_idx < loaded_tweets['tweets'].length; tweet_idx++) {
         ...
         var fake_name = loaded_tweets['tweets'][tweet_idx]['id_str'] + '_fake'
         var ctx = document.getElementById(fake_name);
         var data_array = [];
         var labels = ['false', 'vere'];
         data_array.push(Math.round((parseFloat(loaded_tweets['analysis'][tweet_idx]['misinformation'][1]) + Number.EPSILON) * 100));
         data_array.push(Math.round((parseFloat(1 - loaded_tweets['analysis'][tweet_idx]['misinformation'][1]) + Number.EPSILON) * 100));
